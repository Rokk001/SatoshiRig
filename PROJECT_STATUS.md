...
- Purpose: Neutral Bitcoin solo-mining client with configurable compute backend, Docker/Compose support, and CI/CD.
...
## Usage
- Local: `python -m BtcSoloMinerGpu --wallet <ADDR> [--config ./config/config.toml] [--backend cpu|cuda|opencl] [--gpu 0]`
- Docker: `docker run --rm -e WALLET_ADDRESS=<ADDR> btcsominer-gpu`
